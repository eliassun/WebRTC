...
void sendScreen();
void updateMain(uint64_t code, std::string msg);
void log(uint64_t code, std::string msg);
...
int mainentry(int argc, char **argv) try {
	...
	if (gmode != MODE_SLAVE && gmode != MODE_MASTER) {
        log(codeBaseID, "Run ./picap for dad mode. Run ./picap sweet for sweet mode.");
	    params.usage(EXIT_SUCCESS);
        std::string err = "Wrong mode: " + gmode + ", Expected: " + MODE_SLAVE + " or " + MODE_SLAVE;
        log(codeExit, err);
	    return 0;
	}
	if (gmode == MODE_SLAVE) {
    ...
    log(codeInfo, "Mode: " + gmode + ". Setting up a signalling channel...");
	if (params.noStun()) {
		std::cout
    ...
    if (gmode == MODE_MASTER) {
        log(codeDadID, localId);
    }else {
        log(codeSweetID, localId);
    }
	...
	ws->onOpen([&wsPromise]() {
	    if (gdebug) {
            log(codeInfo, "WebSocket connected");
	    }
		wsPromise.set_value();
        websocketReady = true;
	});
	...
	ws->onError([&wsPromise](std::string s) {
        std::string err = "WebSocket error";
        log(codeCritical, err);
		wsPromise.set_exception(std::make_exception_ptr(std::runtime_error(s)));
	});

	ws->onClosed([]() {
        std::string err = "WebSocket closed";
        log(codeCritical, err);
    });
	...
	if (gmode == std::string(MODE_SLAVE)) {
        log(codeSweetReady, "Sweet is ready. Waiting for a command from dad.");
	    std::cout << "Sweet Mode. https://eliassun.github.io/daddyeye.html for more information." << std::endl;
	    while(true) {
	        sleep(1);
	    }
	}
    log(codeInfo, "Waiting for websocket to be ready...");
    int count = 0;
    while (!websocketReady && count < 5) {
        sleep(1);
        count++;
    }
	//while (true)
...
//		std::cin.ignore();
		...
		dc->onMessage([id, wdc = make_weak_ptr(dc), dc](auto data) {
		    if (std::holds_alternative<std::string>(data)) {
			    ...
			    if (std::get<std::string>(data) == std::string(SEND_START)) {
                    std::string info = "Receiving the sweet's screenshot...";
                    log(codeInfo, info);
			        greceivedBuf.clear();
			    } else if (std::get<std::string>(data) == std::string(SEND_END)) {
                    std::string info = "Decoding the sweet's screenshot...";
                    log(codeInfo, info);
			        decode_image(greceivedBuf);
                    info = "The sweet's screenshot is ready to open. Click \"display\" to open the screenshot. ";
                    log(codeInfo, info);
			    } else {
	...
	//}
    if (gmode == std::string(MODE_MASTER)) {
        log(codeDadReady, "Dad mode is ready.");
...
shared_ptr<rtc::PeerConnection> createPeerConnection(const rtc::Configuration &config,
	...
	pc->onStateChange(
	    [](rtc::PeerConnection::State state) {
            ...
            log(codeInfo, "PeerConnection state is changed");
        });

	pc->onGatheringStateChange([](rtc::PeerConnection::GatheringState state) {
		std::cout << "Gathering State: " << state << std::endl;
        log(codeInfo, "PeerConnection is on Gathering State");
	});
	...
	pc->onLocalDescription([wws, id](rtc::Description description) {
        ...
        log(codeInfo, "PeerConnection is onLocalDescription.");
	});
	...
	pc->onLocalCandidate([wws, id](rtc::Candidate candidate) {
		...
		if (auto ws = wws.lock())
			ws->send(message.dump());
        log(codeInfo, "PeerConnection is onLocalCandidate.");
	});
	...
	pc->onDataChannel([id](shared_ptr<rtc::DataChannel> dc) {
		...
		dc->onOpen([wdc = make_weak_ptr(dc)]() {
            log(codeSweetReady, "Sweet is ready.");
			if (auto dc = wdc.lock())
                ...
                if (msgType == std::string(SEND_CAPTURE)) {
                    if ((gdadid.length() > 0 && dadID == gdadid) || (gdadid.length() == 0)) {
                        if (auto dc = wdc.lock()) {
                            ...
                            std::string screen = capture_screen(gdownloadPath);
...
void csetmode(int mode) {
    std::cout << "setmode: " << mode << std::endl;
    gThread = new std::thread(entry, mode);
}

void cdadmode() {
    std::cout << "dadmode" << std::endl;
    csetmode(0);
}

void csweetmode() {
    std::cout << "sweetmode" << std::endl;
    csetmode(1);
}

void updateMain(uint64_t code, std::string msg) {
    updateMainThread(code, (uint8_t*)msg.c_str(), msg.length());
}
...
void DadEyeWrapper::generateid() {
    localId = randomId(8);
    log(codeMyID, localId);
}

void DadEyeWrapper::dadmode(const char* sweetid) {
    std::cout << sweetid << std::endl;
    gsweetid = std::string(sweetid);
    gThread = new std::thread(entry, 0);
    
}


void DadEyeWrapper::sweetmode(const char* dadid, const char* path) {
    std::cout << dadid << std::endl;
    gdadid = std::string(dadid);
    gdownloadPath = std::string(path);
    gThread = new std::thread(entry, 1);
}

void DadEyeWrapper::getscreen(const char* path) {
    std::cout << "getting screen" << std::endl;
    gdownloadPath = std::string(path);
    gdadmsg = SEND_CAPTURE;
}

void DadEyeWrapper::display(const char* path) {
